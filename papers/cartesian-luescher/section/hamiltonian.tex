...
\end{equation}
\todo{do we want an appendix with the various $\gamma$s in it?}
...
Throughout we focus on a three-dimensional system, though in \Appref{two-d} we study a two-dimensional system.

\begin{figure}
    \includegraphics{example-image-a}
    \caption{\todo{The continuum dispersion relation $\lim_{\epsilon\goesto0}2m\omega^{(\nstep)}(p,\epsilon)$ relation for a variety of $\nstep$ stencils, as well as the quadratic $\nstep=\infty$ relation.  Or something?  There must be a smart way to show this.  Here stick the result of \issue{17}}}
