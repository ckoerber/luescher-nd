...
\end{equation}
where $K$ is a discretized Laplacian, implementing the kinetic energy.

To ensure we control the discretization effects in generality, we study a variety of kinetic operators $K_{xy}$.
An often-used set of finite-difference operators are constructed from the one-dimensional finite-difference Laplacian
\begin{equation}
...
In momentum space the one-dimensional Laplacian may be written
\begin{equation}
    \label{eq:laplacian}
    \Laplacian^{(\nstep)}_{p'p}
    =
...
We can easily fourier transform the Hamiltonian into momentum space, so that
\begin{equation}
