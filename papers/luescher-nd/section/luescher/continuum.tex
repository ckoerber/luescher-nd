...
\caption{Bubble sum. Each line represents a propagator, each vertex represents $-i C(\Lambda)$, and the bubble is given by $I_D$ (see also \Figref{I0}).\label{fig:bubbleSum}}
\end{figure}
...
\begin{equation}\label{eq:IV pole}
    \frac{1}{C(\Lambda)}
    =
    I_D(p) - \frac{\mu}{2 \F_{D0}}\left(\cot \delta_{D0}(p) - i\right)
\end{equation}
...
\begin{equation}\label{eq:spherical zeta}
    \frac{\mu}{2\F_{D0}}(\cot\delta_{D0}(2\mu E)-i) = I_D(2\mu E) - I_{D,\FV}(2\mu E),
\end{equation}
...
\begin{equation}
    \frac{1}{2\F_{D0}}\left(\cot \delta_{D0}(2\mu E) - i\right) = \frac{2}{(2\pi)^2 L^{D-2}}\left[ \left(\int_{\vec{n}} - \sum_{\vec{n}}\right) \frac{1}{x-n^2} + \frac{-i \pi^2\Omega_D}{L} \int \mathrm{d}n\ n^{D-2} \delta\left(\frac{2\pi}{L}n - 2\mu E\right) \right]
\end{equation}
...
\begin{equation}
    \cot \delta_{D0}(p) = \frac{\F_{D0}}{\pi^2 L^{D-2}} \left(\sum_{\vec{n}}-\int_{\vec{n}}\right) \frac{1}{n^2-x}
\end{equation}
...
\begin{equation}\label{eq:spherical quantization}
    \cot \delta_{D0}(p) = \frac{\F_{D0}}{\pi^2 L^{D-2}} S^\spherical_D(x)
\end{equation}
...
In three dimensions it is common to move the momentum dependence in $\F_{D0}$ to the other side, as $p \cot\delta_{D0}(p)$ is what appears in the effective range expansion, as mentioned in \eqref{ere}.
