...
\begin{figure}[h!]
    ...
    \caption{
        Loop diagram contributing to the bubble sum\todo{We need to get rid of the tower of interactions and just replace with $C(\Lambda)$, right?--T.L.}.
    }
    \label{fig:I0}
\end{figure}
...
In a seemingly miraculous (but required) cancellation, the imaginary part on the left hand side exactly cancels the last term in the sum on the right,\footnote{Actually, the cancellation is not \emph{so} miraculous---demanding it occur is essentially how $\F_D$ is determined.}\todo{Hmmm. . . I would prefer not to use the word miraculous, and definitely not use a footnote that later states that it is \emph{not so miraculous}--T.L.} and we are left with
