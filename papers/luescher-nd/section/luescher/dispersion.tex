...
In three dimensions, the sum is divergent and the counter term exactly cancels the this growth; in one dimension there is no divergence to cancel, and we defer the discussion of two dimensions to \Secref{2D}.
...
The structure of the contact interaction is such that it is also possible to incorporate these shifts into a dispersion-aware zeta function.
...
\todo{We provide an explicit derivation in three dimensions in the appendix.}
...
This would be important for capturing physics of physical crystals, were we to match to an infinite volume lattice instead.
