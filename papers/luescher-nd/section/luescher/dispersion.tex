...
As we will show in the next section, it is possible to tune the contact interaction in three dimensions and hence the set of energy levels in the box $x_i$ in such a way that even for finite $N$ the dispersion zeta function $S^{\dispersion N}_{3}( x_i )$ is constant and the $\mathcal O (x/N)$ corrections do not contribute\todo{CK: I am not happy about this statement. It is actually more tricky as discussed in the three-d section. Do you have a better way to formulate this.} when we feed finite-volume finite-spacing energy levels through the finite-spacing quantization condition \eqref{dispersion quantization}.
