...
\caption{
    ...
    Different energy extrapolations in the continuum agree with \Luscher's prediction within uncertainty.
...
We observe that for $x > 5$ all discretization besides the $n_s = \infty$ discretization come with significant uncertainties.
...
This is the case because energy values are close to the poles of the zeta function, e.g., close to the frame boundaries in \figref{continuum-extrapolation-spectrum}.  With imperfect kinetic energy, the lattice artifacts in the energy can push energy levels past a pole in the continuum zeta.  This leads to multiple interacting energy levels on a single segment of the zeta function.
