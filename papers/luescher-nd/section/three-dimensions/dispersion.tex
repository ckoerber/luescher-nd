\subsubsection{The dispersion method in three dimensions}
...
We know want to find an equivalent expression to the FV zeta functions in presence of a discretization scheme $\dispersion$.
...
E.g., the expectation value of the dispersion scales as $\hat K^{(n_s)}(\epsilon) \ket{p} = p^2 [1 + \mathcal O(\epsilon p)^{2 n_s}]\ket{p}$.
...
\begin{equation}\label{eq:dispersion-zeta-contact}
	S^{\dispersion}(x_i)
	=
	S^{(n_s)}(x_i)
	=
	\sum\limits_n^N
	\frac{1}{K_{nn}^{(n_s)} - x_i} - \mathcal{L}_3^{(n_s)} \frac{N}{2}
	\, ,
	\qquad
	\mathcal{L}_3^{(n_s)}
	=
	...
	\, .
\end{equation}
\todo{I am to tired to find out the factor now...}
...
This might not be generally true for other interactions, e.g., if it not possible to make an equivalent choice as in \eqref{dispersion-renormalization}.
...
Then, when we extract finite-volume and finite-spacing energy levels, we put them through the dispersion equation \eqref{dispersion-zeta-contact} using the same $S$ function.
