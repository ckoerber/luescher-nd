...
Nevertheless, we would expect the small $n$ coefficient for larger $n_s$ to be relatively smaller than small $n$ coefficients for smaller $n_s$: $e_{i,n}^{(n_{s_1})} < e_{i,n}^{(n_{s_2})}$ on average for $n_{s_1} > n_{s_2}$.
...
We repeat this procedure for each discretization and compare different continuum energies to decide whether the fits are consistent.
...
We present the model average over the best fits of the spectrum in \figref{continuum-extrapolation-spectrum}.
...
The slope of the extrapolation function changes its sign from $E_2 \to E_4$ for $n_s = 2$ and from $E_6 \to E_8$ for  $n_s = 4$.
...
Independent of the discretization scheme, we observe that the continuum-extrapolated results agree with the constant input phase shifts.
...
Similar to the case where we first extrapolated the spectrum to the continuum and computed phase shifts afterward, the best discretization allows us to also extrapolate higher excited states to zero--visible by the linear log-log dependence of the phase shifts on epsilon.
