...
Note that the lines are not fitted to the data; though the difference is down at $10^{-12}$ or better.
...
The logic of the following derivation remains them same also for $n_s < \infty$, but, in this case, the expressions $\bm n^2$ must be replaced with the proper dispersion $\tilde K^{(n_s)}_{\bm n \bm n}$ \eqref{normalized-kinetic-hamitlonian}, which makes it difficult to obtain closed expressions.  Also, within the Brillouin zone the different dispersion relations cause the two sums to differ by \order{x/N^2} term-by-term.}.
...
In contrast, tuning to the dispersion function directly, as in \Figref{unimproved dispersion}, is flat and nearly zero at each lattice spacing.
